...
void viewer(String j) { // Ряжанка с яблоком, Злагода
  ...
  while (1) {
    ...
    if (displayUpdate != 0) {
      ...
      oled.clear();
      oled.home();
      drawSignal(cof, arr, size, flyx, allTime);  // используем уже полученный arr
      oled.update();
    }
...
void drawSignal(float cof, JsonArray arr, int size, float flyx, uint32_t allTime) {
  ...
  if (percent == 0) percent = 1;  // защита от деления на ноль в map() при сильном зуме
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (arr[i].as<int>() > 0) y = 16;  // Изменяем параметр сигнала если он высокий (.as<int>() — переносимо между версиями ArduinoJson)
